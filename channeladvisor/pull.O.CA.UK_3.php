...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA UK_3 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountUKID3;
$subsidiary = '2';
$mwscompany = '2';		
$salesChannel = '15';
$memo = 'import from ChannelAdvisor Woodworm - UK';        
$currency = '1';
...
// ------------------------------------------------------------  -----------  -------------------------------------------------------------
        ...
        if (!$responseCustomerInsert->writeResponse->status->isSuccess) {
            ...
            echo "--------------------------------------------------------------------------<br>";
            $connector->sendMailWithAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, $responseCustomerInsert->writeResponse->status->statusDetail[0]->message, "UK_3");
            exit();
        } else {
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD SALESORDER ERROR. CA UK_3 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "UK_3");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
	}
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA UK_3 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "UK_3");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA UK_3 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
