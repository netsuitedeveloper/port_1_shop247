...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA FR Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountFRID;
$subsidiary = '2';
$mwscompany = '2';		
$salesChannel = '16';
$memo = 'import from ChannelAdvisor The Sports HQ - FR';        
$currency = '4';
...
// ------------------------------------------------------------  -----------  -------------------------------------------------------------
        ...
        if (!$responseCustomerInsert->writeResponse->status->isSuccess) {
            ...
            echo "--------------------------------------------------------------------------<br>";
            $connector->sendMailWithAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, $responseCustomerInsert->writeResponse->status->statusDetail[0]->message, "FR");
            exit();
        } else {
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD SALESORDER ERROR. CA FR ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "FR");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
	}
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA FR ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "FR");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA FR Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
