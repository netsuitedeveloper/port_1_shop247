...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA EU Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountEUID;
$subsidiary = '2';
$mwscompany = '2';		
$salesChannel = '47';
$memo = 'import from ChannelAdvisor Pixmania EUR';        
$currency = '4';
$asinNumFRCustomField = 'custitem_splc_pixmaniafr';
$asinNumESCustomField = 'custitem_splc_pixmaniaes';
$asinNumITCustomField = 'custitem_splc_pixmaniait';
$asinNumBECustomField = 'custitem_splc_pixmaniabe';
$asinNumDECustomField = 'custitem_splc_pixmaniade';
$asinNumPTCustomField = 'custitem_splc_pixmaniapt';
$accountIndex = '0';
...
//-------------------------------------------------------------------------------------------------------------------------------------
	...
	if ( count($orderArray[$j]->ShoppingCart->LineItemSKUList->OrderLineItemItem) > 1 ){
		...
		foreach ( $orderArray[$j]->ShoppingCart->LineItemSKUList->OrderLineItemItem as $item ){
			...
			$asinCustomField->internalId = $asinNumFRCustomField;
				...
				if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
					...
					if ($searchItemResponse->searchResult->status->isSuccess) {
						if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
							...
							$asinCustomField = new SearchStringCustomField();
							$asinCustomField->searchValue = $item->SKU;
							$asinCustomField->internalId = $asinNumESCustomField;
							$asinCustomField->operator = "contains";    
							$searchItem->basic->customFieldList->customField = array($asinCustomField);
							$requestItemsearch = new SearchRequest();
							$requestItemsearch->searchRecord = $searchItem;
							...
							if ($searchItemResponse->searchResult->status->isSuccess) {
								if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
									...
									$asinCustomField = new SearchStringCustomField();
									$asinCustomField->searchValue = $item->SKU;
									$asinCustomField->internalId = $asinNumITCustomField;
									$asinCustomField->operator = "contains";    
									$searchItem->basic->customFieldList->customField = array($asinCustomField);
									$requestItemsearch = new SearchRequest();
									$requestItemsearch->searchRecord = $searchItem;
									...
									if ($searchItemResponse->searchResult->status->isSuccess) {
										if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
											...
											$asinCustomField = new SearchStringCustomField();
											$asinCustomField->searchValue = $item->SKU;
											$asinCustomField->internalId = $asinNumBECustomField;
											$asinCustomField->operator = "contains";    
											$searchItem->basic->customFieldList->customField = array($asinCustomField);
											$requestItemsearch = new SearchRequest();
											$requestItemsearch->searchRecord = $searchItem;
											...
											if ($searchItemResponse->searchResult->status->isSuccess) {
												if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
													...
													$asinCustomField = new SearchStringCustomField();
													$asinCustomField->searchValue = $item->SKU;
													$asinCustomField->internalId = $asinNumDECustomField;
													$asinCustomField->operator = "contains";    
													$searchItem->basic->customFieldList->customField = array($asinCustomField);
													$requestItemsearch = new SearchRequest();
													$requestItemsearch->searchRecord = $searchItem;
													...
													if ($searchItemResponse->searchResult->status->isSuccess) {
														if ( $searchItemResponse->searchResult->recordList->record['0']->internalId ){
															...
															$asinCustomField = new SearchStringCustomField();
															$asinCustomField->searchValue = $item->SKU;
															$asinCustomField->internalId = $asinNumPTCustomField;
															$asinCustomField->operator = "contains";    
															$searchItem->basic->customFieldList->customField = array($asinCustomField);
															$requestItemsearch = new SearchRequest();
															$requestItemsearch->searchRecord = $searchItem;
		...
		$priority = '1';
		$item = $orderArray[$j]->ShoppingCart->LineItemSKUList->OrderLineItemItem;
		$searchItem = new ItemSearch();
		$asinCustomField = new SearchStringCustomField();
		$asinCustomField->searchValue = $item->SKU;
		$asinCustomField->internalId = $asinNumFRCustomField;
...
// ------------------------------------------------------------  -----------  -------------------------------------------------------------
        ...
        if (!$responseCustomerInsert->writeResponse->status->isSuccess) {
            ...
            echo "--------------------------------------------------------------------------<br>";
            $connector->sendMailWithAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, $responseCustomerInsert->writeResponse->status->statusDetail[0]->message, "EU");
            exit();
        } else {
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD SALESORDER ERROR. CA EU ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "EU");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
	}
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA EU ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "EU");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA EU Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
