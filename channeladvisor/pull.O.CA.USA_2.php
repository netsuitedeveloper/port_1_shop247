...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA USA_2 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountUSAID2;
$subsidiary = '5';
$mwscompany = '3';		
$salesChannel = '18';
$memo = 'import from ChannelAdvisor Voit Golf - US';        
$currency = '2';
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD SALESORDER ERROR. CA USA_2 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA USA_2 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "USA-1");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA USA_2 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
