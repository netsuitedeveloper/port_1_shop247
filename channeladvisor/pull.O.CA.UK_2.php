...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA UK_2 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountUKID2;
$subsidiary = '2';
$mwscompany = '2';		
$salesChannel = '15';
$memo = 'import from ChannelAdvisor hq_clearance - UK';        
$currency = '1';
...
// ------------------------------------------------------------  -----------  -------------------------------------------------------------
        ...
        if (!$responseCustomerInsert->writeResponse->status->isSuccess) {
            ...
            echo "--------------------------------------------------------------------------<br>";
            $connector->sendMailWithAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, $responseCustomerInsert->writeResponse->status->statusDetail[0]->message, "UK_2");
            exit();
        } else {
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD SALESORDER ERROR. CA UK_2 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "UK_2");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
	}
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA UK_2 ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, $response->status->statusDetail[0]->message, "UK_2");
			echo "<br>--------------------------------------------------------------------------<br>";
		}
		$t++;
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA UK_2 Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
