...
echo "--------------------------------------------------------------------------<br>";
echo "Start Import SalesOrder From CA ES Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
...
$accountID = $connector->accountESID;
$subsidiary = '2';
$mwscompany = '2';		
$salesChannel = '24';
$memo = 'import from ChannelAdvisor The Sports HQ - ES';        
$currency = '4';
...
if (!$orderArray || count($orderArray) < 5 ){
	...
	$orderArray[] = $connector->getMissingOrderListFromCA($accountID, 1, $missingArray);
}*/
$missingArray = array("11722191","11722454","11723226");
$orderArray[] = $connector->getMissingOrderListFromCA($accountID, 1, $missingArray);
//------------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------
	...
	$items_count = 0;            
    if($orderArray[$j]->ShoppingCart->LineItemSKUList->OrderLineItemItem->SKU){
...
// ------------------------------------------------------------  -----------  -------------------------------------------------------------
        ...
        if (!$responseCustomerInsert->writeResponse->status->isSuccess) {
            ...
            echo "--------------------------------------------------------------------------<br>";
            $connector->addCustomerErrorAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, json_encode($responseCustomerInsert->writeResponse), "EU");
		...
		if( $matchAddrLine != '-1' && count($customerRec->addressbookList->addressbook) == 1 ){
			...
			if (!$responseCustomerUpdate->writeResponse->status->isSuccess) {
				...
				echo "--------------------------------------------------------------------------<br>";
				$connector->updateCustomerErrorAlert($emailAddress, $orderArray[$j]->ClientOrderIdentifier, json_encode($responseCustomerInsert->writeResponse), "EU");
				$customerInternalId = 'error';
			}
		}
    }
	...
	$customerDepositRecArray[$j]['paymentid']  = $orderArray[$j]->PaymentInfo->PaymentTransactionID;		
...
if ( count($soRecArray) > 0 ){
	...
	foreach ( $responseImportSO->writeResponseList->writeResponse as $response ){
		if ( $response->status->isSuccess ){
			...
			echo "--------------------------------------------------------------------------<br>";
			echo "ADD CUSTOMER DEPOSIT RECORD ERROR. CA ES ID: " .$orderArray[$t]->ClientOrderIdentifier ."<br>";
			if ( $response->status->statusDetail[0]->code != 'JS_EXCEPTION' ){
				$connector->sendMailWithAlert($emailAddress, $orderArray[$t]->ClientOrderIdentifier, json_encode($response), "ES");
			}
			echo "<br>--------------------------------------------------------------------------<br>";
...
echo "--------------------------------------------------------------------------<br>";
echo "End Import SalesOrder From CA ES Into NetSuite Script.<br>";
echo "--------------------------------------------------------------------------<br>";
